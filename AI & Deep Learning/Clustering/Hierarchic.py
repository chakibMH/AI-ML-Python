...
def hierarchic(data,max_d = 220,p = 5,affich_dendo = False):   
    X = data.values
    names = data.index
...
def star_Hie(data_path,pmf_path,formule = 'user-user',max_d = 550,nb_clusters = None):
    #lecture des donnees
    ...
    if formule == 'item-item':
        ...
        clusters,Z = hierarchic(pivot_pmf,max_d)
    elif formule == 'user-user':
        clusters,Z = hierarchic(pmf,max_d)
    else:
    ...
    #enregistrement
    name = "hier_"+formule+"_moy_prec_.pkl"
...
p,r = star_Hie("..\\data\\ratings.csv",pmf_path,formule = "item-item",
          max_d = 580)

#star_Hie("..\\data\\ratings.csv",pmf_path)
